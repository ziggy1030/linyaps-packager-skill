# 兼容性测试（Compatibility Check）和依赖修复工作流

## 概述

本文档描述了 linyaps-packager-skill 中的兼容性测试（compatibility-check）和依赖修复工作流程。该功能基于 `linyaps-pica-helper` 的架构设计，能够在构建失败或运行时检测失败时自动分析和修复依赖问题。
...
#### 1. CompactChecker（兼容性测试器）

**文件**: `scripts/compact_checker.py`
...
**核心方法**:
- `build_with_compact_check_and_auto_fix()` - 执行完整流程
- `_execute_build()` - 执行构建
...
═════════════════════════════════════════════════════
           ...
           Success?                         Failure
                ...
                ↓                               ↓
═════════════════════════════════════════    Next Fix Attempt
   Phase 5: Compatibility Check After Fix
════════════════════════════════════════
                ↓
          ll-builder run (30s timeout)
                ↓
...
files.tar.zst (归档)
    ↓
ll-builder run (compatibility-check)
    ↓
成功 → 导出
失败 → 依赖修复
```
...
- `compact-check-errors/run-error.log` - 兼容性测试错误日志
...
2. **生产环境**
   - 使用 `--skip-compact-check` 加速构建
   - 通过其他方式验证应用功能

3. **调试模式**
   - 使用 `--compact-check-timeout` 增加超时时间
   - 查看 `compact-check-errors/run-error.log` 了解错误详情
...
4. **兼容性测试超时**
   ```
   提示: Compatibility check timed out
   说明: 这是正常的，超时被视为成功
   ```
